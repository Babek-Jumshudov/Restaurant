        ...
        <div class=" b_div_sag">
            ...
            <div class="b_div_sag_products ">
                <div class="container" style="overflow: auto; width: 100% ;height: 83vh;">
                    ...
                    <button
                        style="border: none; background: none; outline: none;position: relative; top: -25%; left: 93%;">

                        <form id="fav_form" action="{{ route('sellers.fav', ['seller' => $seller]) }}" method="GET">
                            @csrf
                            <button
                                style="outline: none; border: none;background: none; position: relative; left: 91%; top: -26%"
                                type="submit">
                                @php
                                    $isFavorite = false;
                                    foreach ($favorites as $favorite) {
                                        if ($favorite->seller_id === $seller->id) {
                                            $isFavorite = true;
                                            break;
                                        }
                                    }
                                ...
                                <i id="fav_icon" style="font-size: 35px;"
                                    class="{{ Request::is('favorites') || $isFavorite ? 'add_fav' : '' }} fa-solid fa-heart">
                                </i>
                    ...
                    <div style="margin-top: -8%">
                        ...
                        <h3> {{ $seller->name }}</h3>
                        <p style="font-size: 14px;color: #888888;">{{ $seller->about }} </p>
                        {{-- ------------------- star ------------------- --}}
                    ...
                    <br>
                    <input class="btn" style="background:#503e9d; color: white" type="button" value="Popular (4)">


                    <div class="images" style="display: flex; justify-content: space-between">
                        @foreach ($products as $product)
                            @if ($product->star >= 5)
                                <div style="display: flex;flex-wrap: wrap;padding: 5px;justify-content: space-between;">
                                    <img class="mt-3" src="{{ asset($product->image) }}"
                                        style="width: 230px; height: 110px; border-radius: 15px;">
                                    <div
                                        style=" display: flex; justify-content: space-between;width: 95%;align-items: baseline;">
                                    ...
                                    <div class="container">
                                        <span
                                            style="font-size: 11px;position: relative;color: #787878;left: -20px; display: flex; align-items: center">
                                            ...
                                            <span style="position: relative; left: -7px">{{ $product->star }}</span>
                                            <svg width="47" height="18" viewBox="0 0 47 18" fill="none"
                                                xmlns="http://www.w3.org/2000/svg">
            ...
            <script>
                document.addEventListener("DOMContentLoaded", function() {
                    ...
                    favIcon.addEventListener("click", () => {
                        if (!isFav) {
                            favIcon.classList.add('add_fav');
                            isFav = true;
                        } else {
                            favIcon.classList.remove('add_fav');
                            isFav = false;
                        }
                    });
